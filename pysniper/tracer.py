...
class Tracer:
    ...
    def __init__(self, output=None, watch=(), watch_explode=(), depth=1,
                 ...
                 color=True, callback=lambda **kwargs: None):
    ...
    def trace(self, frame, event, arg):
        ...
        if self.last_source_path != source_path:
            self.callback(code_event=utils.CodeEvent.NewSourcePath, locals=locals(), source_path=source_path)
        ...
        for name, value_repr in local_reprs.items():
            if name not in old_local_reprs:
                # send to callback
                self.callback(code_event=utils.CodeEvent.NewVar, var_name=name, var_value=value_repr, locals=locals())
                ...
                self.callback(code_event=utils.CodeEvent.ModVar, var_name=name, var_value=value_repr, locals=locals())
                self.write('{indent}{_FOREGROUND_GREEN}{_STYLE_DIM}'
        ...
        if ended_by_exception:
            ...
            self.callback(code_event=utils.CodeEvent.NextSourceLine, source_line=source_line, locals=locals())
